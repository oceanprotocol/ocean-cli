  ...
  it("should return a valid response for 'npm run cli h'", function (done) {
    ...
    exec("npm run cli h", { cwd: projectRoot }, (error, stdout) => {
      ...
      try {
        ...
        expect(stdout).to.contain(
          "editAsset|edit [options] <datasetDid> <metadataFile>",
        );
        expect(stdout).to.contain(
        ...
        expect(stdout).to.contain(
          "startCompute [options] <datasetDids> <algoDid> <computeEnvId> <maxJobDuration> <paymentToken> <resources>",
        );
        expect(stdout).to.contain("Starts a compute job");
        expect(stdout).to.contain(
          "startFreeCompute [options] <datasetDids> <algoDid> <computeEnvId>",
        );
        expect(stdout).to.contain("Starts a FREE compute job");
        expect(stdout).to.contain(
          "stopCompute [options] <datasetDid> <jobId> <agreementId>",
        );
        expect(stdout).to.contain("Stops a compute job");
        expect(stdout).to.contain(
          "getJobStatus [options] <datasetDid> <jobId> [agreementId]",
        );
        expect(stdout).to.contain("Displays the compute job status");
        expect(stdout).to.contain(
          "downloadJobResults <jobId> <resultIndex> [destinationFolder]",
        );
