...
const validateArweave = (input: string) =>
  /^[a-zA-Z0-9_-]{43,}$/.test(input) ||
  "Invalid Arweave transaction ID format.";
const validateURL = (input: string) =>
  /^(https?:\/\/)[^\s/$.?#].[^\s]*$/.test(input) || "Invalid URL format.";

export async function interactiveFlow(
  providerUrl: string,
): Promise<PublishAssetParams> {
  try {
    console.clear();
    console.log(
    ...
    const basicAnswers = await prompt<
      Omit<
        PublishAssetParams,
        "isCharged" | "chainId" | "storageType" | "assetLocation"
      >
    >([
      {
        type: "input",
      ...
      },
      {
        ...
        name: "tags",
        message: chalk.green(
          "Please provide tags to make this asset more easily discoverable (comma separated):\n",
        ),
        required: true,
      },
      {
        ...
        name: "timeout",
        message: chalk.green(
          "After purchasing your asset, how long should the consumer be allowed to access it for?\n",
        ),
        choices: [
    ...
    const { storageType } = await prompt<{
      storageType: PublishAssetParams["storageType"];
    }>({
      type: "select",
      name: "storageType",
    ...
    if (storageType === "ipfs") {
      ...
      assetLocationMessage =
        "Please provide the Arweave transaction ID for your asset:\n";
    ...
    const { chainId } = await prompt<{ chainId: number }>({
      ...
      name: "chainId",
      message: chalk.green(
        "What network will your asset be available for purchase through?\n",
      ),
      choices: [
        { name: "Oasis Sapphire", value: 23294 },
        { name: "Oasis Sapphire Testnet", value: 23295 },
    ...
    if (chainId !== 23295 && chainId !== 23294) {
      const templateAnswer = await prompt<{ template: number }>({
        ...
        choices: [
          {
            name: "Template 1 - user can buy, sell & hold datatokens.",
            value: 1,
          },
          {
            name: "Template 2 - assets are purchased with basetokens and the effective supply of datatokens is always zero.",
            value: 2,
          },
        ],
        result(value) {
