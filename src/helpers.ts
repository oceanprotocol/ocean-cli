...
export async function downloadFile(
  ...
  const defaultName =
    !isNaN(index) && index > -1 ? `file_${index}.out` : "file.out";
  let filename: string;
...
export async function createAsset(
  ...
  let { nftAddress: ddoNftAddress, chainId: ddoChainId } =
  ...
  ddoChainId = parseInt(chainId.toString(10));
  ...
  const datatokenParams: DatatokenCreateParams = {
    ...
    feeToken: ddo?.stats?.price?.tokenAddress
      ? stats.price.tokenAddress
  ...
  let bundleNFT;
  const hasStatsPrice = stats && stats.price; // are price stats defined?
  const hasPriceValue = hasStatsPrice && !isNaN(stats.price.value); // avoid confusion with value '0'
  // no price set
  if (!hasPriceValue) {
    // !ddo.stats?.price?.value
    bundleNFT = await nftFactory.createNftWithDatatoken(
      nftParamsAsset,
      datatokenParams
    );
    // price is 0
  } else if (hasPriceValue && Number(stats.price.value) === 0) {
    // ddo?.stats?.price?.value === "0"
    ...
    const fixedPriceParams: FreCreationParams = {
      ...
      fixedRate: stats.price.value.toString(), // we have a fixed rate price here
      marketFee: "0",
  ...
  if (encryptDDO) {
    ...
    stringDDO = JSON.stringify(remoteDDO);
    const bytes = Buffer.from(stringDDO);
    metadataIPFS = hexlify(bytes);
    flags = 0;
  }
  stringDDO = JSON.stringify(data);
...
export function getIndexingWaitSettings(): IndexerWaitParams {
  ...
  try {
    if (!isNaN(Number(process.env.INDEXING_RETRY_INTERVAL))) {
      indexingParams.retryInterval = Number(
        process.env.INDEXING_RETRY_INTERVAL
      );
