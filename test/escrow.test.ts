...
describe("Ocean CLI Escrow", function () {
  ...
  it("should deposit tokens into escrow", async function () {
    ...
    const initialTokenBalance = await runCommand(
      `npm run cli getUserFundsEscrow ${tokenAddress}`,
    );
    const initialTokenBalanceNumber = initialTokenBalance
      .split(`${tokenAddress}: `)[1]
      .trim();
    ...
    const finalTokenBalance = await runCommand(
      `npm run cli getUserFundsEscrow ${tokenAddress}`,
    );
    const finalTokenBalanceNumber = finalTokenBalance
      .split(`${tokenAddress}: `)[1]
      .trim();
    const numberFinalTokenBalance = Number(finalTokenBalanceNumber);

    expect(output).to.include("Deposit successful");
    expect(numberFinalTokenBalance).to.equal(
      Number(depositAmount) + numberInitialTokenBalance,
    );
  });
  ...
  it("should authorize a payee", async function () {
    ...
    expect(output).to.satisfy(
      (msg: string) =>
        msg.includes("Authorization successful") ||
        msg.includes("already authorized"),
    );
  });
