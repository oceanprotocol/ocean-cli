...
describe("Ocean CLI Paid Compute", function () {
  ...
  it("should get DDO using 'npm run cli getDDO' for compute dataset", async function () {
    ...
    try {
      expect(output).to.contain(
        `Resolving Asset with DID: ${computeDatasetDid}`,
      );
    } catch (error) {
      console.error("Extracted output:", jsonMatch[0]);
      throw new Error("Failed to parse the extracted output:\n" + error);
  ...
  it("should get compute environments using 'npm run cli getComputeEnvironments'", async function () {
    ...
    const jsonMatch = output.match(
      /Existing compute environments:\s*([\s\S]*)/,
    );
    if (!jsonMatch) {
      console.error("Raw output:", output);
  ...
  it("should start paid compute on compute dataset and algorithm with services id for dataset and algorithm", async function () {
    ...
    const serviceIdDataset =
      "ccb398c50d6abd5b456e8d7242bd856a1767a890b537c2f8c10ba8b8a10e6025";
    const serviceIdAlgorithm =
      "db164c1b981e4d2974e90e61bda121512e6909c1035c908d68933ae4cfaba6b0";
    const output = await runCommand(
      `npm run cli -- startCompute ${computeDatasetDid} ${jsAlgoDid} ${computeEnvId} 900 ${paymentToken} '${JSON.stringify(resources)}' 'null' ${serviceIdDataset} ${serviceIdAlgorithm} --accept true`,
    );
  ...
  it("should get job status", async () => {
    const output = await runCommand(
      `npm run cli getJobStatus ${computeDatasetDid} ${computeJobId} ''`,
    );
  ...
  it("should download compute job results", async () => {
    ...
    const output = await runCommand(
      `npm run cli downloadJobResults ${computeJobId} 1 ${destFolder}`,
    );
